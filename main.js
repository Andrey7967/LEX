let Database = [
  ...
  [" censorship "," цензура "],
  [" census "," перепись "],
  [" century "," век "],[" cereal "," злак, каша, хлопья "],
  ...
  [" law "," закон "],
  [" lawn "," газон "],[" lawsuit "," тяжба, судебное дело "],
  ...
  [" tackle "," принадлежности, снаряжение "],[" tag "," этикетка, ярлык "],
  [" tail "," хвост "],
  ...
  function fillWords(first,second) {
      let rand = Math.floor(Math.random() * (Math.abs(second-first)+1)) + first-1 ;
      ...
      for(let i=0;i<4;i++) {
        if(i!=rad) {
          ran =  Math.floor(Math.random() * (Math.abs(second-first)+1)) + first-1;
  ...
  let address = fillWords(fRan,SRan);
